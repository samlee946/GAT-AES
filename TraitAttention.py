...
class MultiNodeGNN(GNN):
    ...
    def forward(self, essay_ids, prompt_ids, essay_input_ids, essay_attention_mask, essay_token_type_ids, essay_feats, labels=None):
        ...
        for i in range(batch_size):
            ...
            # trait attention mechanism with prediction
            logits = []
            for i in range(self.num_traits):
                target_rep = trait_outputs[i].unsqueeze(0).unsqueeze(1)  # Shape: [1, 1, GNN_hidden_size]
                non_target_rep = torch.cat([trait_outputs[:i], trait_outputs[i+1:]], dim=0).unsqueeze(0)  # Exclude the target trait
                rep_att_output = self.trait_attention_layers[i](target_rep, non_target_rep)  # Shape: [1, GNN_hidden_size]
                final_essay_rep = torch.cat([target_rep.view(-1), rep_att_output.view(-1)])
                pred = self.output_layers[i](final_essay_rep)
                logits.append(pred)
